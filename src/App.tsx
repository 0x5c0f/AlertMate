...
export default function App() {
  ...
  const handleSaveState = async (stateToSave = state) => {
    ...
    try {
      ...
      if (res.ok) {
        setSaveMessage('All configurations persistently saved to disk.');
        setTimeout(() => setSaveMessage(null), 3000);
      } else {
        const data = await res.json();
        setSaveError(data.error || 'Failed to save configuration state.');
      }
    } catch (err: any) {
      setSaveError(`Network connection lost: ${err.message || 'Unknown network error'}`);
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className={`min-h-screen flex flex-col items-center justify-center p-6 bg-dots ${isDark ? 'bg-[#050505] text-gray-300' : 'bg-gray-50 text-gray-800'}`}>
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <div className="text-center space-y-2">
            <span className={`text-sm font-semibold block tracking-wide ${isDark ? 'text-white' : 'text-gray-900'}`}>Loading Alertmanager Configurer...</span>
            <span className={`text-xs block ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Retrieving persistent storage files and starting validation server.</span>
          </div>
        </div>
      </div>
    );
    ...
    <div className={`min-h-screen flex flex-col font-sans bg-dots transition-colors duration-200 ${theme === 'dark' ? 'bg-[#09090b] text-gray-300' : 'bg-[#fafafa] text-gray-800'}`}>
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className={`text-base font-semibold tracking-tight ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Alertmanager <span className="text-amber-500">Configurer</span></h1>
              ...
              <p className={`text-xs mt-0.5 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                Visual topology design, inhibit structures, scheduled silences, and hot reload deployment.
              </p>
            </div>
          </div>
          ...
          <div className={`flex items-center gap-4 text-xs font-medium self-start sm:self-center p-2.5 rounded-lg border transition-colors duration-200 ${theme === 'dark' ? 'text-gray-400 bg-[#141416] border-white/10' : 'text-gray-600 bg-gray-50 border-gray-200'}`}>
            <div className={`flex items-center gap-1.5 pr-3 border-r ${theme === 'dark' ? 'border-white/10' : 'border-gray-200'}`}>
              ...
              <span>Receivers: <strong className={`font-mono ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>{state.config.receivers.length}</strong></span>
            </div>
            <div className={`flex items-center gap-1.5 pr-3 border-r ${theme === 'dark' ? 'border-white/10' : 'border-gray-200'}`}>
              <span className="w-2 h-2 bg-amber-500 rounded-full" />
              <span>Routes: <strong className={`font-mono ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>{totalRoutes}</strong></span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 bg-emerald-500 rounded-full animate-ping" />
              <span>Active Silences: <strong className={`font-mono ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>{activeSilencesCount}</strong></span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 self-end sm:self-center">
            ...
            <button
              ...
              title={theme === 'dark' ? '切换至亮色主题' : '切换至暗色主题'}
            >
      ...
      <footer className={`border-t py-4 text-center shrink-0 transition-colors duration-200 ${theme === 'dark' ? 'bg-[#09090b] border-white/10' : 'bg-gray-50 border-gray-200'}`}>
        <span className={`text-[10px] font-mono ${theme === 'dark' ? 'text-gray-500' : 'text-gray-400'}`}>
          Alertmanager Configurer v2.1.0 • Built with Prometheus SRE best practices
        </span>
      </footer>
    </div>
  );
